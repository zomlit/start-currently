...
export const Route = createFileRoute('/_app/dashboard/widgets/visualizer copy')(
  {
...
function VisualizerSection() {
  ...
  React.useEffect(() => {
    if (profiles && profiles.length > 0 && !selectedProfileId) {
      const defaultProfile = profiles.find((p: Profile) => p.settings.isDefault)
  ...
  return (
    <Container>
      <DashboardHeader
        category="Widgets"
        title="Visualizer"
        description="Customize your visualizer settings"
